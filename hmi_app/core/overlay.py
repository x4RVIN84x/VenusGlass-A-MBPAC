...
def _draw_expected_center_guidance(vis, stab_info: dict):
    ...
    Draws:
      ...
      - large operator HUD box on LEFT side so bottom notch edge stays visible
    """
    ...
    # Small label only. No huge extra box attached to the target.
    _put_text(
        vis,
        "TARGET",
        (ex + 16, ey - 18),
    ...
    # ----------------------------
    # Compact operator HUD box
    # ----------------------------
    # Small, bottom-right, away from the notch bottom edge / main geometry.
    box_w = min(360, W - 70)
    box_h = 78 if line2 else 56
    ...
    # Small color tag on left edge.
    cv2.rectangle(vis, (x0, y0), (x0 + 9, y1), (255, 0, 255), -1)
    ...
    if line2:
        put_centered(line1, y0 + 32, 0.58, (255, 255, 255), 2)
        put_centered(line2, y0 + 61, 0.58, (255, 255, 255), 2)
    else:
        put_centered(line1, y0 + 38, 0.60, (255, 255, 255), 2)

...
def _compose_proc_full(raw_bgr: np.ndarray, roi_xywh: Tuple[int, int, int, int], proc_crop_bgr: Optional[np.ndarray]) -> np.ndarray:
    """
    Make PROC view obvious:
      - full frame grayscale background
      - ROI area replaced with processed crop (edges/contrast)
    """
    H, W = raw_bgr.shape[:2]
...
class QCPreviewEngine:
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> QCFrameOutput:
        ...
        # ------------------------------------------------------------
        # Pull detector tuning from cfg (what your Calibration controls save)
        # These knobs are accepted by detector._detect_in_window() :contentReference[oaicite:4]{index=4}
        # ------------------------------------------------------------
        det_kwargs = dict(
        ...
        # ------------------------------------------------------------
        # PROC view
        # detector preprocess debug frames include: gray2_contrast / edges2_close :contentReference[oaicite:5]{index=5}
        # ------------------------------------------------------------
        proc_full = None
        if want_proc and isinstance(dbg, dict):
            dbg_frames = (dbg.get("dbg") or {}) if isinstance(dbg.get("dbg"), dict) else {}
            # Make it obvious: prefer edges
            proc_candidate = (
        ...
        bottom = f"FPS: {self._fps:.1f} | Recipe: {self.recipe.name} | Config: {getattr(self.recipe, 'config_name', 'legacy')}"
        ...
        out = QCFrameOutput(
            overlay_bgr=overlay,
            raw_bgr=raw,
            proc_bgr=proc_full,  # <-- now full-frame PROC
